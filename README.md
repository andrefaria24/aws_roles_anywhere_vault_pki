...
1. Vault creates a root CA plus separate intermediate CAs for workload authentication to AWS and bootstrap certificate authentication to Vault.
2. The signed Vault workload intermediate certificate is used as the AWS IAM Roles Anywhere trust anchor.
...
4. A workload first authenticates to Vault with a bootstrap client certificate through `auth/cert`.
5. Vault issues a short-lived workload certificate with a SPIFFE-style URI SAN such as `spiffe://example/Team1/App1/test`.
6. `aws_signing_helper` exchanges that workload certificate for temporary AWS credentials.
7. The workload uses those temporary credentials to call AWS APIs.
...
- [`vault_agent/`](./vault_agent/): Example VM-oriented Python and PowerShell workflows that use Vault `auth/cert`, issue a workload certificate through Vault Agent templating, and authenticate to AWS IAM Roles Anywhere to list S3 buckets.
